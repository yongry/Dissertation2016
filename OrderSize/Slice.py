...
class OrderManager:
        ...
        for index, row in signals.iterrows():
            if row['Action'] == "Long" or row['Action'] == "Short":
                eachTrade = [row['Code'],
                             ...
                             float(account.getQuantity(row['Strategy'], row['Price'], row['Volume'])) / float(
                                 row['Volume']),
                             row['Price'],
    ...
    def orderPOV(self, signal):
        ...
        while totalTradeSize > 0:
            ...
            if (totalTradeSize - tradableVol < 0):
                orderList.append(
                    [code, tempSearchDate.strftime('%Y-%m-%d %H:%M:%S'), action, totalTradeSize, QntPer, tempPrice,
                     Equity, type])
                totalTradeSize = 0
            else:
                totalTradeSize -= tradableVol
                orderList.append(
                    [code, tempSearchDate.strftime('%Y-%m-%d %H:%M:%S'), action, tradableVol, QntPer, tempPrice, Equity,
                     type])
